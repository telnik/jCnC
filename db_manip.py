...
def usage():
        ...
        print "         -i      --insert        create a new entry"
        print "         -m      --modify        modify an exsisting entry"
        print "         -d      --delete        delete an exsisting entry"
	print "         -r      --retrieve      find a record"
        print "         -h      --host          hostname to use"
        print "         -u      --user          user id to use"
        print "         -p      --pass          passsword to use"
...
def main():
	...
	parser = argparse.ArgumentParser(description="Modify the pyezd database")
	...
	if mode_createdb == 1:
		...
		jdb.insert(hostid, username, password)
		sys.exit(0)
        elif mode_retrieve == 1:
                jdb.retrieve(hostid)
		sys.exit(0)
	elif mode_delete == 1:
		jdb.delete(hostid)
